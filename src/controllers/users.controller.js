...
const update = asyncHandler(async (req, res) => {
  const user = await usersService.update(req.params.id, req, req.body);
  res.json(user);
});

const remove = asyncHandler(async (req, res) => {
  await usersService.deactivate(req.params.id, req);
  res.json({ message: 'Usuário desativado.' });
});

const resetPassword = asyncHandler(async (req, res) => {
  const { senha } = req.body;
  const result = await usersService.resetPassword(req.params.id, req, senha);
  res.json({ message: 'Senha redefinida e sessões encerradas.', ...result });
});

const revokeSessions = asyncHandler(async (req, res) => {
  const result = await usersService.revokeSessions(req.params.id, req);
