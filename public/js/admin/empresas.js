...
function showForm(host, empresa = null) {
  ...
  host.querySelector('form').onsubmit = async ev => {
    ...
    const body = Object.fromEntries([...fd.entries()].filter(([k]) => k !== 'logo'));
    const saved = empresa ? await api.patch(`/api/empresas/${empresa.id}`, body) : await api.post('/api/empresas', body);
    const logo = fd.get('logo');
    if (logo?.size) {
      const up = new FormData(); up.append('logo', logo);
      const res = await fetch(`/api/empresas/${saved.id}/logo`, { method:'POST', headers:{ Authorization:`Bearer ${getToken()}` }, body:up, credentials:'include' });
      if (!res.ok) throw new Error('Empresa salva, mas o upload da logo falhou.');
    }
